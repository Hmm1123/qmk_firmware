...
bool effect_runner_i_reactive_splash(uint8_t start, effect_params_t *params, bg_i_func bg_func, reactive_splash_func reactive_func) {
    ...
    uint8_t time  = scale16by8(g_rgb_timer, qadd8(rgb_matrix_get_bg_speed_scaled() / 4, 1));
    ...
    for (uint8_t i = led_min; i < led_max; i++) {
        ...
        for (uint8_t j = start; j < count; j++) {
            ...
            uint16_t tick = scale16by8(g_last_hit_tracker.tick[j], qadd8(rgb_matrix_get_reactive_speed_scaled(), 1));
